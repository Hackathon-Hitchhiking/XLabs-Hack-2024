...
def resume_for_inference(resume, exp_dir, model, device):
    if resume is not None:
        checkpoint = resume
        logging.info(f"Resume from the provided checkpoint {resume}")
    else:
        ...
        if len(ckpts) == 0:
            ...
            ckpts.sort(key=lambda x: os.stat(str(x)).st_ctime)
            checkpoint = str(ckpts[-1])
            logging.info(f"Automatically resuming from the latest checkpoint {checkpoint}")

    # Load the checkpoint and filter out incompatible keys
    state_dict = torch.load(checkpoint, map_location=device)['codec_model']
    state_dict = {k.split("module.")[-1] if k.startswith("module.") else k: v
                  for k, v in state_dict.items()}

    # Filter out only matching keys with the model's state_dict
    model_state_dict = model.state_dict()
    compatible_state_dict = {k: v for k, v in state_dict.items()
                             if k in model_state_dict and model_state_dict[k].shape == v.shape}

    # Update model's state_dict with compatible weights from the checkpoint
    model_state_dict.update(compatible_state_dict)
    model.load_state_dict(model_state_dict, strict=False)

    logging.info(f"Loaded {len(compatible_state_dict)} matching parameters from checkpoint")
