...
SingingVoice_format = {
    'keys': ["text_seq", "instruction_seq", "audio_seq"],
    'type': ["text", "text_t5", "audio"],
    'features': [],
    'loss_key': 'audio_seq',
    'encoder_keys': ["text_seq", "instruction_seq"],
}
...
task_formats['singing_voice'] = SingingVoice_format
...
def load_data_for_one_task(dataset_json):
    ...
    for key, data_type in zip(task_format['keys'], task_format['type']):
        # Skip 'audio_seq' key if it is the output and does not need an input file
        if key == 'audio_seq':
            continue
